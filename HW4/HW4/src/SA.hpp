...
inline Record M1_move(HBStarTree<ll>& HB_tree, vector<NodeBase<ll>*>& HB_node){
    ...
    NodeBase<int64_t>* picked_node = pick_node(HB_node);
    while(!picked_node->parent || picked_node->kind == NodeBase<ll>::Kind::Hierarchy) picked_node = pick_node(HB_node); //先不挑 root + Hierarchy node
